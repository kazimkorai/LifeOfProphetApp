...
public class DBManagerSahihBukhari extends SQLiteOpenHelper {
    ...
    public static String DATABSE_NAME = "Sahih_Bukhari_Database";
     ...
     * @param context
     */
    public DBManagerSahihBukhari(Context context) {
    ...
    public boolean checkIsBookMark(int pagePos, String chapterName) // check to set the bookMark image
    {
        try {

            String checkBookMark = "  Select Bookmarks from Sahih_Hadith  WHERE Chapter_Names= '" + chapterName + "' and Hadith_Number=" + pagePos;

            Cursor cursor;
            cursor = getCursor(checkBookMark);
            if (cursor != null) {
                cursor.moveToFirst();
                do {
                    bookMarkValue = Integer.parseInt((cursor.getString(cursor.getColumnIndex("Bookmarks"))));
                }
                while (cursor.moveToNext());
            }
            cursor.close();
        } catch (Exception e) {
           /* if(toastBookmark!=null) {
                toastBookmark.cancel();
            }*/
            //  toastBookmark = Toast.makeText(_context, "Exception ocurred in  checking Bookmark ", Toast.LENGTH_SHORT);
            //  toastBookmark.show();
        } finally {
...
//	   }
    ...
    public DataModel getQueryByChapterNameAndId(String idSearch,String chapterNameSearch) {
